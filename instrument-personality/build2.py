instruments = {
  ...
  pipa: {id:'pipa',img:'inst-pipa.png',name:'琵琶',persona:'烈弦者',color:'#b5343a',colorLight:'#c94a50',traits:['敢爱敢恨','情绪饱满','热烈奔放','不藏不掖'],description:'你像一杯烈酒，入口惊艳，让人难忘。你的人生大起大落都写在脸上，对朋友掏心掏肺，对生活全力以赴。敢爱敢恨是你的底色，永不平庸是你的宿命。如同琵琶自西域传入中原，你身上有一种文化交融的多元气质——热烈、丰富、永远出人意料。',poetry_text:'白居易《琵琶行》"大弦嘈嘈如急雨，小弦切切如私语。嘈嘈切切错杂弹，大珠小珠落玉盘。"——千古名篇，将琵琶的丰富表现力写到了极致。琵琶自西域传入中原，本身就是文化交融的象征，热烈而多元。',poetry_source:'——白居易《琵琶行》',pieces:['《十面埋伏》','《霸王卸甲》','《春江花月夜》'],figures:['李白 — 豪放不羁，诗酒趁年华','苏轼 — 大起大落，永远热爱生活']},
  guqin: {id:'guqin',img:'inst-guqin.png',name:'古琴',persona:'素弦者',color:'#576574',colorLight:'#6b7d8e',traits:['清雅通透','内心笃定','享受孤独','独立思考'],description:'你是人群中的隐士，不争不抢，但所有人都尊重你的判断。你享受孤独，甚至需要孤独来充电。你的力量不在表面的热烈，而在内心的笃定——你知道自己要什么，也知道什么不值得。如同古琴七弦，素朴无华，却是君子修身养性之物，是士人精神的至高象征。',poetry_text:'伯牙子期《高山流水》——琴为知音而弹，知音既去，摔琴绝弦。古琴文化中最动人的就是这种"士为知己者死"的精神。《诗经》"窈窕淑女，琴瑟友之"，琴也是君子修养的象征。王维"独坐幽篁里，弹琴复长啸"，琴与隐逸、与山水、与自我修行密不可分。',poetry_source:'',pieces:['《高山流水》','《广陵散》','《平沙落雁》'],figures:['陶渊明 — 归隐田园，不为五斗米折腰','嵇康 — 刑场上弹完《广陵散》，从容赴死']},
  dizi: {id:'dizi',img:'inst-dizi.png',name:'笛子',persona:'清风者',color:'#4a7c59',colorLight:'#5d9670',traits:['自由洒脱','浪漫气质','不喜束缚','随性而行'],description:'你像一阵穿堂风，所到之处都是清爽。你不喜欢被束缚，无论是人际关系还是生活方式，你都需要足够的空间。你有一种天然的浪漫气质，别人眼里的"不靠谱"其实是你对世界的独特感知方式。如同笛声入春风，吹遍一座城——你的感染力不需要声嘶力竭，只需轻轻一吹。',poetry_text:'李白《春夜洛城闻笛》"谁家玉笛暗飞声，散入春风满洛城。此夜曲中闻折柳，何人不起故园情。"笛声入春风，吹遍一座城——这就是笛子的穿透力和感染力。向秀《思旧赋》"听鸣笛之慷慨兮，妙声绝而复寻"，笛声也承载着故人之思。',poetry_source:'——李白《春夜洛城闻笛》',pieces:['《姑苏行》','《鹧鸪飞》','《扬鞭催马运粮忙》'],figures:['李白 — 一生好入名山游，千金散尽还复来','唐伯虎 — 别人笑我太疯癫，我笑他人看不穿']},
...
var currentQ=0,scores={erhu:0,pipa:0,guqin:0,dizi:0,guzheng:0,suona:0},audioPlaying=null;

function startQuiz(){document.body.classList.remove('on-home');showPage('quiz');renderQuestion()}

function goHome(){document.body.classList.add('on-home');showPage('home')}
...
function renderQuestion(){var q=questions[currentQ],html='<div class="question-card" id="q'+q.id+'"><div class="question-text">'+q.text+'</div>';if(q.type==='text'){html+='<div class="options-list">';q.options.forEach(function(o,i){html+='<button class="option-btn" onclick="selectOption('+q.id+','+i+')"><span class="option-label">'+o.label+'</span><span class="option-text">'+o.text+'</span></button>'});html+='</div>'}document.getElementById('questionArea').innerHTML=html;document.getElementById('qCurrent').textContent=currentQ+1;document.getElementById('progressFill').style.width=((currentQ)/questions.length*100)+'%'}

function selectOption(qid,optIdx){var q=questions.find(function(q){return q.id===qid});var opt=q.options[optIdx];Object.keys(opt.scores).forEach(function(inst){scores[inst]+=opt.scores[inst]});currentQ++;if(currentQ<questions.length){renderQuestion()}else{showResult()}}

function showResult(){var maxScore=0,winner='erhu';Object.keys(scores).forEach(function(inst){if(scores[inst]>maxScore){maxScore=scores[inst];winner=inst}});var inst=instruments[winner];var total=Object.values(scores).reduce(function(a,b){return a+b},0);var html='<div class="result-container"><div class="result-card"><div class="result-header" data-inst="'+inst.id+'"><div class="result-instrument-icon"><img src="'+inst.img+'" alt="'+inst.name+'"></div><div class="result-instrument-name">'+inst.name+'</div><div class="result-persona-title">'+inst.persona+'</div></div><div class="result-body"><div class="result-section"><div class="result-section-title">人格解读</div><p>'+inst.description+'</p></div><div class="result-section"><div class="result-section-title">古韵诗词</div><p>'+inst.poetry_text+inst.poetry_source+'</p></div><div class="result-section"><div class="result-section-title">代表曲目</div><ul>';inst.pieces.forEach(function(p){html+='<li>'+p+'</li>'});html+='</ul></div><div class="result-section"><div class="result-section-title">知音古人</div><ul>';inst.figures.forEach(function(f){html+='<li>'+f+'</li>'});html+='</ul></div><div class="result-section"><div class="result-section-title">性格标签</div><div class="result-traits">';inst.traits.forEach(function(t){html+='<span class="trait-tag">'+t+'</span>'});html+='</div></div><div class="score-chart"><div class="score-bar-row"><div class="score-bar-label">二胡</div><div class="score-bar-track"><div class="score-bar-fill" style="width:'+(scores.erhu/total*100)+'%;background:#2c3e6b"></div></div></div><div class="score-bar-row"><div class="score-bar-label">琵琶</div><div class="score-bar-track"><div class="score-bar-fill" style="width:'+(scores.pipa/total*100)+'%;background:#b5343a"></div></div></div><div class="score-bar-row"><div class="score-bar-label">古琴</div><div class="score-bar-track"><div class="score-bar-fill" style="width:'+(scores.guqin/total*100)+'%;background:#576574"></div></div></div><div class="score-bar-row"><div class="score-bar-label">笛子</div><div class="score-bar-track"><div class="score-bar-fill" style="width:'+(scores.dizi/total*100)+'%;background:#4a7c59"></div></div></div><div class="score-bar-row"><div class="score-bar-label">古筝</div><div class="score-bar-track"><div class="score-bar-fill" style="width:'+(scores.guzheng/total*100)+'%;background:#8b8682"></div></div></div><div class="score-bar-row"><div class="score-bar-label">唢呐</div><div class="score-bar-track"><div class="score-bar-fill" style="width:'+(scores.suona/total*100)+'%;background:#c4932a"></div></div></div></div></div><div class="result-actions"><button class="btn-retry" onclick="retry()">重新测试</button><button class="btn-share" onclick="share()">分享结果</button></div></div></div>';document.getElementById('resultArea').innerHTML=html;document.body.classList.remove('on-home');showPage('result')}
...
function share(){var text='我在弦外之音测试中是「'+instruments[Object.keys(scores).reduce(function(a,b){return scores[a]>scores[b]?a:b})]+'」，快来测测你是哪种乐器吧！';if(navigator.share){navigator.share({title:'弦外之音',text:text})}else{navigator.clipboard.writeText(text).then(function(){alert('结果已复制到剪贴板！'})})}}
